...
bsub -W 08:00 -o "${CURRENTTARGET}VirusScreen_Cluster_With_SVM_01_$(date +"%y%m%d%H%M%S").results" "matlab -nodisplay -nojvm << M_PROG
Create_CellTypeClassificationPerColumn_iBRAIN('$CURRENTTARGET')
VirusScreen_Cluster_With_SVM_01('$CURRENTTARGET');
TraditionalPostClusterInfectionScoring_With_SVM('$CURRENTTARGET');
Create_CellTypeOverview_iBRAIN('$CURRENTTARGET');
M_PROG"
