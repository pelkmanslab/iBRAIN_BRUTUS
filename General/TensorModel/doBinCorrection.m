function [matCompleteDataBinReadout,matBinEdges,matBinDimensions,matTensorII,matTensorTCN] = doBinCorrection(matCompleteData, strFinalFieldName, varargin)
%
% help for doBinCorrection
%
% Usage:
%
% [matCompleteData, strFinalFieldName, matCompleteMetaData, matCompleteDataBinReadout] = doBinCorrection(matCompleteData, strFinalFieldName, matCompleteMetaData, varargin)
%
...
if boolUserSuppliedMaxBins
    % get the max number of bins from the most likely candidata in varargin
...
boolUserSuppliedBinEdges = any(cellfun(@(x) isnumeric(x) & size(x,2)==size(matCompleteData,2) & size(x,1)<24, varargin));
...
if boolUserSuppliedBinEdges
    ...
    matBinEdges = varargin{cellfun(@(x) size(x,2)==size(matCompleteData,2), varargin)};
else
