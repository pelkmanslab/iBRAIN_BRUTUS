function [bs, ps, rs, matAllCombis] = recalculateModelWithDimensions(TrainingData,W_threshold)
...
matAllCombis = all_possible_combinations2(ones(1,intNumOfDims-1)+1)-1;

intNumOfCombis = size(matAllCombis,1);

bs = nan(intNumOfCombis,intNumOfDims); %model-params
ps = nan(intNumOfCombis,intNumOfDims); %p-values
rs = nan(1,intNumOfCombis); %r-squared values
yt = TrainingData(:,1); %model readout

for iDim = 2:intNumOfCombis%first row is all zeros...
...
%             end
    ...
    % STORE RESULTS
    matCurrentColumns(1)=1;
    bs(iDim,matCurrentColumns) = b';
    ps(iDim,matCurrentColumns) = stats.p';
end
