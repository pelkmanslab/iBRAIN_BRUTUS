...
% if nargin == 0
    strRootPath = '\\Nas-biol-imsb-1\share-2-$\Data\Users\Philip\iBRAIN2\50K_Tfn\';
% end
    ...
    for iCols = 2:11
        for iSwitch = 1:2
            ...
            for iOligo = iOligoCount
                ...
                if iSwitch == 1
                    matCurWellCellIndices = find(MasterTensor.MetaData(:,1) == iRows & MasterTensor.MetaData(:,2) == iCols & MasterTensor.MetaData(:,3) == iOligo);
                ...
                if length(matCurWellCellIndices) > 50
                    ...
                    for ii = 1:size(matAllPresentCombinations,1)
                        matCurrentClass = repmat(matAllPresentCombinations(ii,:)],size(matTrainingData,1),1);                        

                        [matCurrentRowIndices, foo] = find(sum(matTrainingData(:,2:end) == matCurrentClass,2) == size(matCurrentClass,2));
