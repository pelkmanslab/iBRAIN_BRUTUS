function handles = IdentifyPrimLoG2wGlobalStaticMaxRescaling(handles)
...
%textVAR04 = MaxQuant (will not matter in analysis)
%defaultVAR04 = 0.90
...
%textVAR08 = Should spots be areas or only maxima?
%choiceVAR08 = Area above Threshold
%choiceVAR08 = Only Local Maxima
sSpotDefintion = char(handles.Settings.VariableValues{CurrentModuleNum,8});
%inputtypeVAR08 = popupmenu

%textVAR09 = Fixed Maximum Intensity
%defaultVAR09 = 400
sDivIntens = char(handles.Settings.VariableValues{CurrentModuleNum,9});
...
settings.minQuantileImgIntensity= str2double(sMinQuant);
settings.maxQuantileImgIntensity=str2double(sMaxQuant);
settings.ImgThresholdUpMin = str2double(sImgThresholdUpMin); %565;
settings.Radius = str2double(sRadius);
settings.ThresholdForSpots = str2double(sThresholdForSpots); 



Threshold = settings.ThresholdForSpots;
...
OrigImage = double(CPretrieveimage(handles,ImageName,ModuleName,'DontCheckColor','DontCheckScale')).*65535;


%OrigImage = double(CPretrieveimage(handles,ImageName,ModuleName,'MustBeGray','CheckScale'));
Radius = settings.Radius;
%OrigImage2=imread('Z:\Data\Users\mRNAmes\iBrain\110801WashPreAmpVariations\TIFF\110801Wash_C13_s10_w25786B9F1-243B-49FB-B964-885B68F5BF3B.png');
...
ImgThresholdMax = quantile(OrigImage(:),settings.maxQuantileImgIntensity);

    if ImgThresholdMax < settings.ImgThresholdUpMin
        ImgThresholdMax = settings.ImgThresholdUpMin;
    end  

drawnow

im = double(OrigImage) - double(ImgThresholdMin);
if any(im(:))
    im = im / double(ImgThresholdMax);
end
...
% According to user input spot objects are either defined as every region
% above the threshold or as the the local maxima, which are above the
% threshold

switch sSpotDefintion
    
    
    case 'Area above Threshold'
        % Definie Spots as continuos pixels, which are above the threshold
        bw=ac>Threshold;
        
    case 'Only Local Maxima'
        % Only obtain single pixel, which corresponds to local maximum.
        % This definition is also used by the original IdentifyPrimLoG2
        ...
        if any(ac(:))
            ...
            r=ac;
            r(~bw)=0;
              %  indices = find(imregionalmax(r));
            bw=imregionalmax(r);
            % maxima = sortrows([indices r(indices)], -2);
            %bw(maxima(:,1)) = true;
    ...
    otherwise
        fprintf('No proper definition on how spots are defined, please select.');
...
%                     'Callback', @CP_ImagePopupmenu_Callback);
  ...
  text(0.1,-0.08,...
      ['Threshold: ' num2str(mean(Threshold(:))) ', Number of objects: ' num2str(max(FinalLabelMatrixImage(:)))],...
      'Color','black',...
