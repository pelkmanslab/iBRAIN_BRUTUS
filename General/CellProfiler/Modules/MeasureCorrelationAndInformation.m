function handles = MeasureCorrelationAndInformation(handles)

% Help for the Measure Correlation And Information module:
...
for i = 1:ImageCount-1
    for j = i+1:ImageCount
        CorrelationFeatures{end+1} = ['Corr_',ImageName{i},'_',ImageName{j}];
        InformationFeatures{end+1} = ['MutInfo_',ImageName{i},'_',ImageName{j}];
    end
end
...
for ObjectNameNbr = 1:ObjectNameCount
    ...
    matMutualInformation = zeros(NbrOfObjects,length(CorrelationFeatures));   % Pre-allocate memory
...
%                 try
                    if isempty(index) || numel(index) == 1 % If the object does not exist in the label matrix or is only one pixel, the correlation calculation will fail, so we assign the correlation to be NaN.
                        ...
                        MutualInformationForCurrentObject = 0;
                    else
                        ...
                        % [BS HACK] Also calculate MutualInformation
                        MutualInformationForCurrentObject = information(double(Image{i}(index))',double(Image{j}(index))');             % Get the values for these indexes in the images and calculate the mutual information
                    ...
                    matMutualInformation(ObjectNbr,FeatureNbr) = MutualInformationForCurrentObject; % Store the mutual information
...
%                 end
    ...
    if strcmp (ObjectName{ObjectNameNbr},'Image')
        handles.Measurements.(ObjectName{ObjectNameNbr}).CorrelationFeatures = [CorrelationFeatures InformationFeatures SlopeFeatures];
        handles.Measurements.(ObjectName{ObjectNameNbr}).Correlation(handles.Current.SetBeingAnalyzed) = {[Correlation matMutualInformation Slope]};
    else
        handles.Measurements.(ObjectName{ObjectNameNbr}).CorrelationFeatures = [CorrelationFeatures InformationFeatures];
        handles.Measurements.(ObjectName{ObjectNameNbr}).Correlation(handles.Current.SetBeingAnalyzed) = {[Correlation matMutualInformation]};
    end
end
