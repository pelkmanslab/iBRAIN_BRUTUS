...
for LocalMaximaTypeNumber = 1:length(LocalMaximaTypeList)
    for WatershedTransformImageTypeNumber = 1:length(WatershedTransformImageTypeList)
        ...
        if strcmp(LaplaceValues,'/')
            ...
            %%% Merge oversegmented nuclei (MF added 110620)            
            Objects = Merge_Nuclei_New (Objects, MaxDiameter);
        ...
        if strcmp(OriginalLocalMaximaType,'None') || (strcmp(OriginalLocalMaximaType,LocalMaximaType) && strcmp(OriginalWatershedTransformImageType,WatershedTransformImageType))
            ...
            [tmpXXX,tmpExcludedBorderObjects] = bwlabel(BorderObjects > 0);
            clear tmpXXX;
            ...
            if isempty(column)
                ...
                handles.Measurements.Image.ObjectDiameterFeatures(end+1) = {['BorderObjects ' ObjectName]};
            end
            ...
            handles.Measurements.Image.ObjectDiameter{handles.Current.SetBeingAnalyzed}(1,column+4) = tmpExcludedBorderObjects;
...
%MatF subfunction for merging after calculation of compactness%%
%Adapted from ''Automated Segmentation, Classi?cation, and Tracking of
% Cancer Cell Nuclei in Time-Lapse Microscopy
% Xiaowei Chen, Xiaobo Zhou, and Stephen T. C. Wong''

function Objects = Merge_Nuclei_New (NewObjects, MaxDiameter)

ObjectsAfterWS  = unique(NewObjects);
NewLabelCompactness = [];
ClosestNeighbour = [];
IndexOfNewPerim = [];


NewObjectsAreaCentersPerim = regionprops(NewObjects, 'Area', 'Centroid','Perimeter'); 

for i = ObjectsAfterWS';
    if i~=0;                                                                               %keep only the labeled object i and sum all the pixels, basically this is the perimeter, here no need of bwperim, the WS is already giving a single pixel boundary
    IndexOfNewPerim(i) = NewObjectsAreaCentersPerim(i).Perimeter ;                                      %Store the perimeter in an array that will be used later.   
    end                                                                            
end


%definition of centers and areas of  new objects%
%%Note: the for loop allows to handle centers more easily, indeed region
%%props creates a structured Array hard to handle in loops
% CenterNewObjects = cat(1,Centers.Centroid);

for i = ObjectsAfterWS';
    if i~=0;                                                                   
    CenterNewObjects(i,1) = NewObjectsAreaCentersPerim(i).Centroid(1,1);
    CenterNewObjects(i,2) = NewObjectsAreaCentersPerim(i).Centroid(1,2);
    end                                                                            
end
                                                                               %regionprops look @ the labeled 2D array NewObjects and give the centers of each labeled objects with the option ''centroid'' 


%Let's calculate the compactness of each objects after the WaterShed

for i = ObjectsAfterWS';
    if i ~= 0;
       NewLabelCompactness(i) = (IndexOfNewPerim(i)^2)/(4.0*pi*NewObjectsAreaCentersPerim(i).Area);
    end
end

FinalImage = zeros(size(NewObjects));
Index = 0;
Index2 = 0;
%easier : i am not looking at the old objects anymore%

for i = ObjectsAfterWS';
               if i ~= 0;
               VectorX = CenterNewObjects(:,1) - CenterNewObjects(i,1);
               VectorY = CenterNewObjects(:,2) - CenterNewObjects(i,2);
               VectorLgthSqr = (VectorX.^2)+(VectorY.^2);                                
               VectorLgthSqr(VectorLgthSqr==0)= NaN ;                                   
               [ClosestNeighbour(i,2),ClosestNeighbour(i,1)] = nanmin(VectorLgthSqr);                     %Distance between the two centers in ClosestNeighbour(i,2), label of the closest neighbour in ClosestNeighbour(i,1)
               end
end
               
%%Merging of closest neighbours & calculation of compactness%%

 for i = ObjectsAfterWS'
    if i ~= 0;
        if ClosestNeighbour(i,2) < (0.65*MaxDiameter)^2;
        
        TmpImage = (NewObjects == i)+(NewObjects == ClosestNeighbour(i,1));                                %The resulting image here is a logical image made of the two ojects to merge
        TmpImage = bwlabel(TmpImage);
        z = size(unique(TmpImage));
        TmpImage = double(TmpImage);                                                                      
        
                     
        if z(1,1) == 3 ;
        TmpImage = conv2(TmpImage,ones(3),'same');
        TmpImage = logical(TmpImage);
        
            TmpImageTosubstrct = bwperim(TmpImage);
            TmpImage = TmpImage - TmpImageTosubstrct;
            TmpParam = regionprops(TmpImage, 'Perimeter', 'Area');
            try
            Compactness = (TmpParam(1).Perimeter^2)/(4.0*pi*TmpParam(1).Area);
            catch
            Compactness = (2*NewLabelCompactness(i))+1;
            end
                 if Compactness < NewLabelCompactness(i)
                    %disp('doing it')
                    Index = Index + 1 ;
                    ClosestNeighbour(ClosestNeighbour(i,1),2) = (2*MaxDiameter)^2;                         %I avoid here the cross merging cause of the if ClosestNeighbour(i,2) < (0.5*MaxDiameter)^2; statement
                    FinalImage = FinalImage + TmpImage; 
                 else
                    Index2 = Index2 + 1 ;
                    %disp('NOT doing it')
                 end
...
FinalImage = FinalImage + NewObjects;

Objects = bwlabel(FinalImage);
