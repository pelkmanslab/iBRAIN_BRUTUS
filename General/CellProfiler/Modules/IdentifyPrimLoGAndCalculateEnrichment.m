function handles = IdentifyPrimLoGAndCalculateEnrichment(handles)
...
%textVAR01 = What did you call the images you want to detect vesicles on?
%infotypeVAR01 = imagegroup
ImageName = char(handles.Settings.VariableValues{CurrentModuleNum,1});
%inputtypeVAR01 = popupmenu

%textVAR02 = What do you want to call the objects identified by this module?
%defaultVAR02 = Vesicles
%infotypeVAR02 = objectgroup indep
ObjectName = char(handles.Settings.VariableValues{CurrentModuleNum,2});

%textVAR03 = What did you call the parent object in which vesicles should fall?
%infotypeVAR03 = objectgroup
ParentObjectName = char(handles.Settings.VariableValues{CurrentModuleNum,3});
%inputtypeVAR03 = popupmenu

%textVAR04 = Typical diameter of objects, in pixel units:
%defaultVAR04 = 5
Radius = char(handles.Settings.VariableValues{CurrentModuleNum,4});

%textVAR05 = Score threshold for match.  Enter a number, leave as 'Automatic', or adjust the Automatic threshold with a multiplicative correction factor (separating comma is necessary), e.g. 'Automatic,1.2'.
%defaultVAR05 = Automatic
ThresholdStr = char(handles.Settings.VariableValues{CurrentModuleNum,5});

%%% ADDED PARAMETERS FOR ENRICHMENT SCORING

%textVAR06 = On what images do you want to calculate the signal enrichment?
%infotypeVAR06 = imagegroup
EnrichmentImageName = char(handles.Settings.VariableValues{CurrentModuleNum,6});
...
EnrichmentImage = double(CPretrieveimage(handles,EnrichmentImageName,ModuleName,'MustBeGray','CheckScale'));

Radius = str2double(Radius);

ParentLabelMatrixImage = CPretrieveimage(handles,['Segmented', ParentObjectName],ModuleName,'DontCheckColor','DontCheckScale',size(OrigImage));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% ADDED VESICLE ENRICHMENT CALCULATIONS %%%


% discard any dot that's not inside a Parent Object segmentation (cells)
bw = bw .* logical(ParentLabelMatrixImage);

% get reference dots, i.e. pseudo-random dots, evenly spaced at 1.5 * Radius interval
% 1) make neatly ordered grid
reference_dots = false(size(im));
reference_dots(1:round(Radius*1.5):end,1:round(Radius*1.5):end) = true;
% calculate new dot positions with a 1 pixel random 'jitter', up&down,
% left&right 
[dotX,dotY] = find(reference_dots);
dotX = dotX + round(((rand(size(dotX))>0.5)*2)-1);
dotY = dotY + round(((rand(size(dotY))>0.5)*2)-1);
% redraw map with jitter
reference_dots = false(size(im));
matBadIX = dotX<=0 | dotY<=0 | dotX>size(im,1) | dotY>size(im,2);
matIndX=sub2ind(size(im),dotX(~matBadIX),dotY(~matBadIX));
reference_dots(matIndX) = true;

% remove reference dots outside of Parent segmentation
reference_dots = reference_dots .* logical(ParentLabelMatrixImage);

% remove reference_dots that are too close to vesicle peaks (i.e. within
% Radius of LoG peaks 
reference_dots = reference_dots .* ~imdilate(bw, strel('disk', Radius));

% calculate per cell, the average of the log2 ( vesicle intensity / average
% ( local (?) reference dots ) )

% init some variables
intCurrentImage = handles.Current.SetBeingAnalyzed;
intObjectCountIX = find(strcmpi(handles.Measurements.Image.ObjectCountFeatures,ParentObjectName));
intNumOfParentObjects = handles.Measurements.Image.ObjectCount{intCurrentImage}(intObjectCountIX);

% init measurement to store. rows are ParentObject, columns are:
% 1) parent vesicle enrichment score 
% 2) num of vesicles 
% 3) num of reference dots 
...
for iParent = 1:intNumOfParentObjects
    ...
    cur_parent_reference_dots = logical(reference_dots) .* logical(ParentLabelMatrixImage==iParent);
    

    
    % get vesicle location
    [matVesicleI,matVesicleJ] = find(cur_parent_bw);
    numVesDots = size(matVesicleI,1);
    
    % get reference location
    [matReferenceI,matReferenceJ] = find(cur_parent_reference_dots);
    numRefDots = size(matReferenceI,1);

    % store number of dots, vesicles & reference
    matEnrichmentScorePerParent(iParent,2) = numVesDots;
    matEnrichmentScorePerParent(iParent,3) = numRefDots;
    
    if numVesDots==0 || numRefDots == 0
        continue
    end

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% calculate the vesicle enrichment scores %%%
    
    % all pairwise euclidean distances [#ref, #ves]
    matDistances = sqrt( ...
        (repmat(matVesicleI',numRefDots,1) - repmat(matReferenceI,1,numVesDots)).^2 + ...
        (repmat(matVesicleJ',numRefDots,1) - repmat(matReferenceJ,1,numVesDots)).^2 ...
    );

    [foo,matIX]=sort(matDistances,1);
    clear foo
    % get the 5 (or less, if not enough reference dots available) indices
    % of reference_dot_locations clostest to each vesicle 
    matClosestSubIndicesPerVesicle = sub2ind(size(bw),matReferenceI(matIX(1:min(numRefDots,5),:)),matReferenceJ(matIX(1:min(numRefDots,5),:)));
    
    % pixel values of vesicle dots
    matVesicleValues = double(EnrichmentImage(sub2ind(size(bw),matVesicleI,matVesicleJ))');
    
    % pixel values of top-4 closest reference dots (row) per vesicle (column)
    matMeanClosestReferenceValues = mean(double(EnrichmentImage(matClosestSubIndicesPerVesicle)),1);
    ...
    % all pairwise euclidean distances [#ref, #ref]
    matDistances = sqrt( ...
        (repmat(matReferenceI',numRefDots,1) - repmat(matReferenceI,1,numRefDots)).^2 + ...
        (repmat(matReferenceJ',numRefDots,1) - repmat(matReferenceJ,1,numRefDots)).^2 ...
    );
    
    % set self distances to Inf
    matDistances(logical(eye(size(matDistances)))) = Inf;
    ...
    % pixel values of vesicle dots
    matVesicleValues = double(EnrichmentImage(sub2ind(size(bw),matReferenceI,matReferenceJ))');
    ...
    % store average log2 ratio of vesicle-peak vs average of closest 5
    % reference dots
    matEnrichmentScorePerParent(iParent,4) = median(log2(matVesicleValues ./ matMeanClosestReferenceValues));
    
    % difference between true enrichment and backgroun enrichment
    matEnrichmentScorePerParent(iParent,5) = matEnrichmentScorePerParent(iParent,1) - matEnrichmentScorePerParent(iParent,4);
...
%     colormap(gray)
%     subplot(2,2,1)
%     imagesc(cur_parent_bw)
%     title('vesicle dots')
%     subplot(2,2,2)
%     imagesc(cur_parent_reference_dots)
%     title('reference dots')
%     subplot(2,2,3)
%     hist(matEnrichmentScorePerParent(:,1))
%     title('backgroun corrected enrichment score for all cells')
%     subplot(2,2,4)
%     hist(matEnrichmentScorePerParent(:,5))
%     title('background corrected enrichment score for all cells')
%     drawnow
%     suptitle(sprintf('%s',mfilename))
%     drawnow
...
% store final measurement results. rows are ParentObject, columns are:
% 1) parent vesicle enrichment score 
% 2) num of vesicles 
% 3) num of reference dots 
% 4) parent reference enrichment score
% 5) vesicle enrichment - reference enrichment
handles.Measurements.(ParentObjectName).(CPjoinstrings('Enrichment',[ObjectName,ImageName,EnrichmentImageName,'Features'])) = {'Enrichment_Raw','Vesicle_Count','Reference_Count','Enrichment_Random','Enrichment_Corrected','Vesicle intensity'};
handles.Measurements.(ParentObjectName).(CPjoinstrings('Enrichment',[ObjectName,ImageName,EnrichmentImageName])){intCurrentImage} = matEnrichmentScorePerParent;
% handles = CPaddmeasurements(handles, ParentObjectName, CPjoinstrings(ObjectName,[ImageName,EnrichmentImageName,'EnrichmentRaw']), matEnrichmentScorePerParent(:,1));
% handles = CPaddmeasurements(handles, ParentObjectName, CPjoinstrings(ObjectName,[ImageName,EnrichmentImageName,'Count']), matEnrichmentScorePerParent(:,2));
% handles = CPaddmeasurements(handles, ParentObjectName, CPjoinstrings(ObjectName,[ImageName,EnrichmentImageName,'ReferenceCount']), matEnrichmentScorePerParent(:,3));
% handles = CPaddmeasurements(handles, ParentObjectName, CPjoinstrings(ObjectName,[ImageName,EnrichmentImageName,'EnrichmentRandom']), matEnrichmentScorePerParent(:,4));
% handles = CPaddmeasurements(handles, ParentObjectName, CPjoinstrings(ObjectName,[ImageName,EnrichmentImageName,'EnrichmentCorrected']), matEnrichmentScorePerParent(:,5));

%%% END OF: ADDED VESICLE ENRICHMENT CALCULATIONS %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if any(findobj == ThisModuleFigureNumber)
    ...
    dilatedVesicles = imdilate(bw, strel('disk', 2));
%     dilatedReferences = imdilate(reference_dots, strel('disk', 2));
%     dilatedVesicles = bw;
    dilatedReferences = reference_dots;

    intLowIntensity = quantile(OrigImage(:),0.025);
    intHighIntensity = quantile(OrigImage(:),0.975)    ;
    r = (OrigImage - intLowIntensity) /  (intHighIntensity - intLowIntensity);
    g = (OrigImage - intLowIntensity) /  (intHighIntensity - intLowIntensity);
    b = (OrigImage - intLowIntensity) /  (intHighIntensity - intLowIntensity);
    r(logical(dilatedVesicles)) = 1;
    g(logical(dilatedVesicles)) = 0;
    b(logical(dilatedVesicles)) = 0;
    r(logical(dilatedReferences)) = 0;
    g(logical(dilatedReferences)) = 1;
    b(logical(dilatedReferences)) = 0;
    visRGB = cat(3, r, g, b);
