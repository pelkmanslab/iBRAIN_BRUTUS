...
% 02. 03. 2009 some modification was done to handle no vesicle case
% 18. 03. 2009 Zeropixel Bug fixed
...
%    intVal
        ...
        if num_vesicles > 0
            ...
            relco =d2co/ max(d2co);
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind}, relco, 'Relative_dist_from_outline_to_');
            relcc =d2cc/ max(d2cc);
        
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind},relcc, 'Relative_dist_from_centre_to_');
            ...
            rad30 = VescPercent2Rad(  v_label , 0.30);
            rad60 = VescPercent2Rad(  v_label , 0.60);
        
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind},rad30, 'Radius_for_covering_30_Percent_');
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind},rad60, 'Radius_for_covering_60_Percent_');
        
            count18 = Rad2VescCount(  v_label , 18);
            count25 = Rad2VescCount(  v_label , 25);
        
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind},count18, 'Vesicle_within_18Pixel_from_');
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind},count25, 'Vesicle_within_25Pixel_from_');
        ...
        else % if there is no vesicle
            measurementNode{ind} = struct;
            measurementNode{ind} = constructSingleMeasurementNode( measurementNode{ind},0, 'Number_of_', 'sum' ,'Total');
            measurementNode{ind} = constructSingleMeasurementNode( measurementNode{ind},0, 'Per_U_Carea_Number_of_', 'sum' ,'Total');
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind},zeros(0,0), 'Size_of_');
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind},zeros(0,0), 'Intensity_of_');
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind},zeros(0,0), 'Distance_from_Cell_outline_to_');
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind},zeros(0,0), 'Distance_from_Cell_centre_to_');
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind}, zeros(0,0), 'Relative_dist_from_outline_to_');
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind},zeros(0,0), 'Relative_dist_from_centre_to_');
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind},zeros(0,0), 'Distance_from_Fluorescent_centre_to_');
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind},zeros(0,0), 'Distance_Between_');
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind},zeros(0,0), 'Minimum_Intensity_of_Individual_');
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind},zeros(0,0), 'Maximum_Intensity_of_Individual_');
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind},zeros(0,0), 'Radius_for_covering_30_Percent_');
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind},zeros(0,0), 'Radius_for_covering_60_Percent_');
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind},zeros(0,0), 'Vesicle_within_18Pixel_from_');
            measurementNode{ind} = constructMeasurementNode(measurementNode{ind},zeros(0,0), 'Vesicle_within_25Pixel_from_');
        end
    else %% if there is no cell area
       measurementNode{ind} = struct;
       measurementNode{ind} = constructSingleMeasurementNode( measurementNode{ind},0, 'Number_of_', 'sum' ,'Total');
       measurementNode{ind} = constructSingleMeasurementNode( measurementNode{ind},0, 'Per_U_Carea_Number_of_', 'sum' ,'Total');
       measurementNode{ind} = constructMeasurementNode(measurementNode{ind},zeros(0,0), 'Size_of_');
...
% save(['ccc' num2str( round(rand*1000) )],'measurementNode');