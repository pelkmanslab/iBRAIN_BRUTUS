function [matTotal, matInfected, matWellPresent] = ConvertHandlesToPlate(handles)
...
% plate.
%             [matTotal, matInfected, matWellPresent] =
%             ConvertHandlesToPlate(handles)
    ...
    for rowNum = rowstodo %2:7
        ...
        for colNum = colstodo %2:11
            ...
            for k = find(~cellfun('isempty',FileNameMatches))
                ...
                if boolVirusData
                    intImgTotalInfected = intImgTotalInfected + handles.Measurements.(strObjectName).VirusInfection{k}(:,TotalInfectedIndex);
                end

            end
            
            
            matWellPresent(rowNum,colNum) = ~isempty(k);
            
            matTotal(rowNum,colNum) = intImgTotal; 
            if boolVirusData
                matInfected(rowNum,colNum) = intImgTotalInfected;
            end
        end
    end
end