...
numSDs1 = 500
load(fullfile(strRootPath,sprintf('BASICDATA_%dSDs.mat',numSDs1)))
BASICDATA1 = BASICDATA;

numSDs2 = 1500
load(fullfile(strRootPath,sprintf('BASICDATA_%dSDs.mat',numSDs2)))
...
%         disp(sprintf('analyzing oligo %d',iOligo))        
        matReplicaIndices1 = find(BASICDATA1.PlateNumber(:,1)==iPlate & BASICDATA1.OligoNumber(:,1)==iOligo);
        matReplicaIndices2 = find(BASICDATA2.PlateNumber(:,1)==iPlate & BASICDATA2.OligoNumber(:,1)==iOligo);        
        for i = matReplicaIndices1'
            for ii = matReplicaIndices2'

%                 if i > ii
                if BASICDATA1.ReplicaNumber(i,1) ~= BASICDATA2.ReplicaNumber(ii,1)

                    % only look at those wells that had siRNAs targeting genes in them
                    matIndices = matGeneIDs(i,:) > 0;

                    matCorrs1 = [matCorrs1, corr(BASICDATA1.ZScore(i,matIndices)',BASICDATA2.ZScore(ii,matIndices)','rows','complete')]; %#ok<AGROW>
                    matCorrs2 = [matCorrs2, corr(BASICDATA1.CellTypeOverviewZScoreLog2NonOthersII(i,matIndices)',BASICDATA2.CellTypeOverviewZScoreLog2NonOthersII(ii,matIndices)','rows','complete')]; %#ok<AGROW>
                    matCorrs3 = [matCorrs3, corr(BASICDATA1.Log2RelativeCellNumber(i,matIndices)',BASICDATA2.Log2RelativeCellNumber(ii,matIndices)','rows','complete')]; %#ok<AGROW>
                    ...
                    matPlateCorrelations(matCurNaNIndx,numPlateIndx) = corr(BASICDATA1.ZScore(i,matIndices)',BASICDATA2.ZScore(ii,matIndices)','rows','complete');
                    matPlateCorrelations2(matCurNaNIndx,numPlateIndx) = corr(BASICDATA1.CellTypeOverviewZScoreLog2NonOthersII(i,matIndices)',BASICDATA2.CellTypeOverviewZScoreLog2NonOthersII(ii,matIndices)','rows','complete');                    
                    matPlateCorrelations3(matCurNaNIndx,numPlateIndx) = corr(BASICDATA1.Log2RelativeCellNumber(i,matIndices)',BASICDATA2.Log2RelativeCellNumber(ii,matIndices)','rows','complete');                                        
                end
...
% gcf2pdf(strRootPath,sprintf('BASICDATA_PlateCorrelations_%dSDs_vs_%dSDs',numSDs1,numSDs2))
