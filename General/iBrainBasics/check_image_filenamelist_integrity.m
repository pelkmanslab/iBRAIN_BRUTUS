function check_image_filenamelist_integrity(strRootPath)

    warning off all;

    if nargin == 0
        tic
        disp('Run_PostClusterLocalDensity: 50K MODE')
        if ispc
...
%         path = strcat(strRootPath,RootPathFolderList{folderLoop,1},filesep);
        ...
        if boolFileNameListFound
            ...
            strImagePath = strrep(path,'\DATAFUSION','');            
...
%                             disp(sprintf('       : OK %s IN %s',char(strImageFile),getlastdir(path)))                            
            ...
            check_image_filenamelist_integrity(path);
        end
