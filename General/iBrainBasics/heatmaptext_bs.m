function  heatmaptext_bs(data,varargin)
...
if nargin > 1
    ...
    okargs = {'precision','fontcolor','colorbar','xticklabel','yticklabel','colormapcentered','textvalues'};
    for j=1:2:nargin-2
        pname = varargin{j};
        pval = varargin{j+1};
        ...
        elseif length(k)>1
            ...
            switch(k)
                ...
                case 3
                    cBar = pval;
                case 4
                    cellstrXTickLabel = pval;
                case 5
                    cellstrYTickLabel = pval;
                case 6
                    boolColormapCentered = pval;
                case 7
                    matTextValues = pval;
            end
        end
...
if boolColormapCentered
    datatemp = double(data);
    datatemp(isinf(datatemp))=  NaN;
    datamax = nanmax(abs(datatemp(:)));
    im = imagesc(data,[-datamax datamax]);
else
    im = imagesc(data);
end
...
% allow user to specify labels for both axis
if ~isempty(cellstrXTickLabel)
    set(imAxes,'XTick',1:length(cellstrXTickLabel) + 0.5)
    set(imAxes,'XTickLabel',cellstrXTickLabel,'fontsize',7)
else
    set(imAxes,'XTick',[])
    set(imAxes,'XTickLabel',[])
end
if ~isempty(cellstrYTickLabel)
    set(imAxes,'YTick',1:length(cellstrYTickLabel) + 0.5)
    set(imAxes,'YTickLabel',cellstrYTickLabel,'fontsize',7)
else
    set(imAxes,'YTick',[])
    set(imAxes,'YTickLabel',[])
end
if isempty(cellstrYTickLabel) && isempty(cellstrYTickLabel)
   axis off 
end
...
for i = 1:rows
    for j = 1:cols
                 textHandles(j,i) = text(j,i,num2str(matTextValues(i,j),precision),...
                'color',textColor,'horizontalAlignment','center',...
                'fontsize',fs,'clipping','on','visible','off','fontweight','bold');
    end
end
if  showText 
