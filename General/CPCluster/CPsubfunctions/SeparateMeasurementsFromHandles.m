...
for ii = 1:length(ListOfMeasurements)
    ...
    if length(ListOfMeasurements{ii}) > 6 && ... 
        ...
        OutPutFile = fullfile(strDataSorterOutputFolderPath, sprintf('%s%d_to_%d_Measurements_%s_%s.mat',BatchFilePrefix,StartImage,EndImage,char(ListOfObjects(i)), char(ListOfMeasurements(ii))));
