function Create_CellTypeClassificationPerColumn_iBRAIN_BlobsOnly(strPath)
...
if not(boolInputPathExists)
    ...
    disp(sprintf('\n%s: starting on %s',mfilename,strPath))
end
...
for i = 1:length(classes)
    [handles, class_list{i}, numPositiveClassNumbers(i)] = load_latest_svm_file(handles, strPath, classes{i});
end
...
for iImage = 1:length(handles.Measurements.SVM.(class_list{1}))
    ...
    if not(isempty(strfind(strPath,'VV')))
...
%         matApoptoticIndices = handles.Measurements.SVM.(class_list{3}){iImage} == numPositiveClassNumbers(3);
        ...
        matInterphaseIndices = ones(size(matBlobIndices));
        matMitoticIndices = zeros(size(matBlobIndices));
        matApoptoticIndices = zeros(size(matBlobIndices));
