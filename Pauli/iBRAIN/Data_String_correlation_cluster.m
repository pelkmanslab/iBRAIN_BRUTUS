% Data String correlation
function Data_String_correlation_cluster(threshold)
...
strPathStringPreCalc=npc('/BIOL/imsb/fs2/bio3/bio3/Data/Users/Pauli/String_apriori_precalc/');

disp('Loading MetaData...')
load([strPath,'MetaData.mat']);
MetaData.AssayNames{8}='PseudoData';

disp('Loading OligoData...')
load([strPath,'Data_oligo.mat']);

disp('Loading PseudoData...')
load([strPath,'PseudoData_oligo.mat']);

for assay=1:length(MetaData.AssayNames)
    for target=1:length(MetaData.ReadoutNames)
        for type=1:length(MetaData.String.EdgeTypes)
            load([strPathStringPreCalc,'ConnectionsTriang_',num2str(threshold),'_',num2str(type)])
       
            disp([MetaData.AssayNames{assay},', ',MetaData.ReadoutNames{target},', ',MetaData.String.EdgeTypes{type},', ',num2str(threshold)])
            
            for gene=1:MetaData.genes
                
                if assay==8
                    GeneID=MetaData.GeneID.(MetaData.AssayNames{1})(gene);
                else
                    GeneID=MetaData.GeneID.(MetaData.AssayNames{assay})(gene);
                end
                index=find(MetaData.String.GeneIDs==GeneID);
                if isempty(index)
                    point(gene,1)=NaN;
                    point(gene,2)=NaN;
                else
                    ...
                    if assay==8
                        gene_value=nanmean2(PseudoData_oligo.(MetaData.ReadoutNames{target})(gene,:));
                    else
                        gene_value=nanmean2(Data_oligo.(MetaData.AssayNames{assay}).(MetaData.ReadoutNames{target}).(MetaData.ReadoutBaselLevel3{target})(gene,:));
                    end
                    neighbors=find(ismember(MetaData.String.DG_stringID,ConnectionsTriang.(MetaData.String.EdgeTypes{type}).(['T',num2str(threshold)]){index})); % USE TRIANG
                    
                    neighbor_GeneIDs=MetaData.String.GeneIDs(neighbors);
                    if assay==8
                        neighbor_indices=find(ismember(MetaData.GeneID.(MetaData.AssayNames{1}),neighbor_GeneIDs));
                    else
                        neighbor_indices=find(ismember(MetaData.GeneID.(MetaData.AssayNames{assay}),neighbor_GeneIDs));
                    end
                    if assay==8
                        neighbor_values=nanmean2(PseudoData_oligo.(MetaData.ReadoutNames{target})(neighbor_indices,:)');
                    else
                        neighbor_values=nanmean2(Data_oligo.(MetaData.AssayNames{assay}).(MetaData.ReadoutNames{target}).(MetaData.ReadoutBaselLevel3{target})(neighbor_indices,:)');                  
                    end
                    neighbor_value(1)=nansum2(neighbor_values);
                    %neighbor_value(2)=nansum2(neighbor_values)./sqrt(length(neighbor_values));
                    %neighbor_value(3)=nansum2(neighbor_values)./length(neighbor_values);
                    
                    point(gene,1)=gene_value;
                    point(gene,2)=neighbor_value(1);
                end
            end
            
            point(point==0)=NaN;
            try
                good=not(isnan(point(:,1))|isnan(point(:,2)));
            catch
                good=[];
            end
            ...
            corrResults(assay,target,type)=corr2(x,y,'rows','pairwise');
            pointsResults(assay,target,type)=length(x);
...
disp(['Saving results'])
save([strPathTemp,'corrResults_',num2str(threshold),'.mat'],'corrResults');
save([strPathTemp,'pointsResults_',num2str(threshold),'.mat'],'pointsResults');
disp(['Results saved'])

%--------------
function m = nanmean2(x,dim)
%nanmean2 Mean value, ignoring NaNs.
%   M = nanmean2(X) returns the sample mean of X, treating NaNs as missing
%   values.  For vector input, M is the mean value of the non-NaN elements
%   in X.  For matrix input, M is a row vector containing the mean value of
%   non-NaN elements in each column.  For N-D arrays, nanmean2 operates
%   along the first non-singleton dimension.
%
%   nanmean2(X,DIM) takes the mean along dimension DIM of X.
%
...
%-----------
function [coef, pval] = corr2(x, varargin)
...
%
% subplot(2,1,1)
% plot(corrResults{assay}{target})
% legend(EdgeTypes,'Location','NorthEastOutside')
% axis([1 length(EdgeThresholds) 0 1])
% set(gca,'XTickLabel',100:100:1000)
% xlabel('Threshold')
% ylabel('Correlation')
%
% subplot(2,1,2)
% plot(pointsResults{assay}{target})
% legend(EdgeTypes,'Location','NorthEastOutside')
% axis([1 length(EdgeThresholds) 0 7000])
% set(gca,'XTickLabel',100:100:1000)
% xlabel('Threshold')
% ylabel('Genes')
%
% set(gcf,'Position', [147 481 1577 617])
%
% figure
% hold on
% for i=1:length(EdgeTypes)
%     plot(pointsResults{assay}{target}(:,i),corrResults{assay}{target}(:,i))
% end
% axis([0 7000 0 1])
