...
if nargin==0
    strRootPath = '/BIOL/imsb/fs3/bio3/bio3/Data/Users/VSV_DG/070328_VSV_DG_batch2_CP032-1ac/BATCH/';
...
for iImage = 1:length(handles.Measurements.Image.FileNames)
    if not(isempty(class_list{1})) && not(isempty(class_list{2})) && not(isempty(class_list{3})) && not(isempty(class_list{4}))
        ...
        matInterphaseIndices=ind{5}|ind{6}|ind{7}|ind{8};%|ind{13}|ind{14}|ind{15}|ind{16};
        matMitoticIndices=ind{3}|ind{4};%|ind{11}|ind{12};
        matApoptoticIndices=ind{2};%|ind{10};
        matOthers=ind{1}|ind{9}|ind{10}|ind{11}|ind{12}|ind{13}|ind{14}|ind{15}|ind{16}; %use also green channel to correctly detect OOF images
