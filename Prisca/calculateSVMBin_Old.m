...
function [normal_vector,classification_accuracy,b,average_margin] = calculateSVMBin_Old(gene_name,C,sample_size)
...
test_cases2=arrayfun(@(x) randi([size(treated_population,1)+1 size(population,1)],sample_size1,1),1:100,'UniformOutput',false);
tic
                                for(cross=1:100) %Select 10 times random training and tesing samples, the solution belonging to the median classification accuracy is returned
                                    ...
                                    train = [randi2([1 ceil(size(treated_population,1)/2)],sample_size1,1);randi([size(treated_population,1)+1 size(population,1)],sample_size1,1)];
...
b(cross)=0;%model.w(length(feature_size)+1);
...
%arrayfun(@(x)
%ttest3(test_cases3{x},test_cases_control{bin}{x},[],'right','unequal'),1:100)
% end;
