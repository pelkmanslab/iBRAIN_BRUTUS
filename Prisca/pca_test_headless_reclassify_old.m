...
classdef pca_test_headless_reclassify_old < handle
    properties
        ...
        output1;
        output2;
        pos_pop; %Number of pertrubed cells being farther way than the support vectors
        neg_pop;%Number of control cells being farther way than the support vectors
        calculate_normals;
    ...
    methods
        function obj=pca_test_headless_reclassify_old(root_path,config1_path,fullfile_path,num_p,num_c,input_path)
 ...
 obj.num_cells=num_c;
        ...
        obj.calculate_normals=true;
         ...
         obj.feature_names=temp_features(:,NUM_POPULATION_FEATURES:23);
       ...
       obj.normal_vectors= Normal_vector_matrix_bin_after_bin;
            obj.bias=bias;
...
%                 end;
               ...
               c_crit=obj.c_crit;
                       Gene_list=obj.gene_names;
                        ...
                        save(write_path, 'Gene_list','Classification_accuracy','c_crit');
...
features1=features;
                ...
                for(fc=1:20)
                out1=hist(features(col_cells,fc)-nanmedian(features(col_cells,fc)),quantile(features(:,fc),[0.002]):diff_a(fc):quantile(features(:,fc),0.98))/length(features(col_cells,fc));
             ...
             if(nanmean(diff2)<nanmean(diff1))
                 sprintf('Column %d corrected feature %d',col,fc)
                            ...
                            features1(col_cells,fc)=features(col_cells,fc)-nanmedian(features(col_cells,fc));
                end;
                end;
                        end;
                ...
                median_r(row,:)=nanmedian(features(row_cells,:));
                            for(fc=1:20)
                out1=hist(features(row_cells,fc)-nanmedian(features(row_cells,fc)),quantile(features(:,fc),[0.002]):diff_a(fc):quantile(features(:,fc),0.98))/length(features(row_cells,fc));
             diff2=abs(out1-out2{fc});
             ...
             if(nanmean(diff2)<nanmean(diff1))
                 sprintf('Row %d corrected feature %d',row,fc)
                             ...
                             features1(row_cells,fc)=features(row_cells,fc)-nanmedian(features(row_cells,fc));
     ...
     [bootstraped_population]=(find(abs(features(sorted_coefficients(:),1))<2));
                obj.output=[];obj.output1=NaN(length(obj.gene_names),6,1138);obj.output2=[];obj.normal_vector=[];
                obj.pos_pop=[];obj.c_crit=NaN(length(obj.gene_names),6,1138);
                obj.neg_pop=[];
...
obj.coefficient_variation(gene,bin)=sum(std(treated_population,0,2)./abs(mean(treated_population,2)));
                                ...
                                if((size(treated_population,1)<obj.num_cells))
                                 ...
                                 sample_size=size(treated_population,1);
                                            test_cases1=arrayfun(@(x) population([randi([1 size(treated_population,1)],sample_size,1)],1:20),1:100,'UniformOutput',false);

test_cases2=arrayfun(@(x) population([randi([size(treated_population,1)+1 size(population,1)],sample_size,1)],1:20),1:100,'UniformOutput',false);
                                for(score_svm=1:1138)
                                    %Loop over all support vector machines
                                    %of the bin
                            if(size(obj.normal_vectors,1)>0)
                         
                            
feature_size=1:20;

   sample_size=size(treated_population,1);
   w=obj.normal_vectors(score_svm,(bin-1)*20+1:(bin-1)*20+20);
   b=obj.bias(score_svm,bin);
        
  test_cases3=arrayfun(@(x)  test_cases1{x}*w'+b,1:50,'UniformOutput',false);

test_cases4=arrayfun(@(x) test_cases2{x}*w'+b,1:50,'UniformOutput',false);


test_class=arrayfun(@(x) (length(find(test_cases3{x}>0))+length(find(test_cases4{x}<0)))/(2*sample_size),1:50,'UniformOutput',true);

  c_criteria=arrayfun(@(x) sum(max(0,1-(test_cases3{x}))),1:50,'UniformOutput',true)/(2*sample_size)+arrayfun(@(x) sum(max(0,1+test_cases4{x})),1:50,'UniformOutput',true)/(2*sample_size)   ;     
        
% if(ttest(c_criteria(1:50),1,0.01)==1)
    %Signficamt classification
    obj.c_crit(count,bin,score_svm)=mean(c_criteria);
   obj.output1(count,bin,score_svm)=mean(test_class);
% end;
