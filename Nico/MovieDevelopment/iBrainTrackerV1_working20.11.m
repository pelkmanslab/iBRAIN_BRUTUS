...
% for a visualization movie. Also you can write 'only' if you have all the frames but you want
% to create a movie out of them. Default =Default = 'Yes'.
...
% correction not used
% PhyTree: Wether the family trees should be calculated or not
%
...
%VZ
movieOnly = strncmpi(handles.TrackingSettings.CreateFrames,'o',1);
...
getPhyTrees = strncmpi(handles.TrackingSettings.PhyTrees,'y',1);
...
for iSites = 1:numTotalSites
    ...
    %[MF] I have extended the outputs of this function for the need of the
    %'over multiple site' tracking.
    
    [handles, TrackingMeasurementPrefix] = BasicTrackerV1(handles,cellAllSegmentedImages,matOrderedTimePointIdx,[matRows, matColumns, matSites ,matTimepoints],strSettingBaseName);
    %changed by vito:     [handles, matglobalLabelingSeq, TrackingMeasurementPrefix] = BasicTrackerV1_vito(handles,cellAllSegmentedImages,matOrderedTimePointIdx,[matRows, matColumns, matSites ,matTimepoints],strSettingBaseName);
   ...
   if getPhyTrees
       ...
       %[VZ] Here I calculate the family trees
  handles =  createfamilytree(handles,matOrderedTimePointIdx,strSettingBaseName,iSites);
   end
...
% commented out by vito handles = TrackOvermultiplesitesV1(handles, TrackingMeasurementPrefix, matglobalLabelingSeq, cellAllSegmentedImages, matRows, matColumns, matSites ,matTimepoints, strWells, imageSize, threshold_merge);
...
if getPhyTrees
    handles.Measurements.(strObjectToTrack).(strcat(TrackingMeasurementPrefix,'PhyTreeForm')) = PreviousHandles.Measurements.(strObjectToTrack).PhyTreeForm;
    handles.Measurements.(strObjectToTrack).(strcat(TrackingMeasurementPrefix,'TreeNodeInfo')) = PreviousHandles.Measurements.(strObjectToTrack).TreeNodeInfo;
end
