...
%             Yanic: .pvalue: Pvalues of cluster retained
...
clustersMMF = repmat(struct('position',[],'amplitude',[],'bgAmp',[],...
    'numDegFree',[],'residuals',[],'pvalue',[]),numClusters,1);
...
if ~isempty(indx)
    ...
    for iCluster = 1 : length(clustersMMF)
        ...
        clustersMMF(iCluster).pvalue=pValue;
...
if visual
    %yh:changed visualization so something is visible
    %make 3 layers out of original image (normalized)
    imageNorm = image/quantile(image(:),0.97);
