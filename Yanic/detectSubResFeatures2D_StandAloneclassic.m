...
if exist([imageDir filenameBase enumString(imageIndx(1),:) '.png'],'file')
    imageTmp = imread([imageDir filenameBase enumString(imageIndx(1),:) '.png']);
else
...
for iImage = 1 : numImagesRaw
    if exist([imageDir filenameBase enumString(imageIndx(iImage),:) '.png'],'file')
...
for iImage = last5start : numImagesRaw
    ...
    if imageExists(iImage)
        imageLast5(:,:,i) = imread([imageDir filenameBase ...
            enumString(imageIndx(iImage),:) '.png']);
    end
end
...
if absBG
    imageTmp = imread([bgImageDir bgImageBase enumString(imageIndx(1),:) '.png']);
        ...
        for jImage = 1 : 1 + 2*integWindow(iWindow)
            if imageExists(jImage+iImage-1)
                imageRaw(:,:,jImage) = double(imread([imageDir filenameBase ...
                    enumString(imageIndx(jImage+iImage-1),:) '.png']));
            end
        end
...
%because they are clearly an arpngact of time-integration
...
imageIntegF1=imageIntegF;
imageIntegF1(sub2ind(size(imageIntegF),localMaxPosX,localMaxPosY))=1;
imagesc(imageIntegF1)
...
if numSigmaIter
    ...
    while numIter <= numSigmaIter && acceptCalc && ((psfSigma-psfSigma0)/psfSigma0 > 0.05)
        ...
        for iImage = images2use
            ...
            %read raw image
            imageRaw = imread([imageDir filenameBase enumString(imageIndx(iImage),:) '.png']);
