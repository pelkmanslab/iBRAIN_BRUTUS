...
bsub -W 34:00 -o $(dirname $batchjob)/$REPORTFILE -R 'rusage[mem=4000]' "matlab -singleCompThread -nodisplay -nojvm << M_PROG
CPCluster('$(dirname $batchjob)/Batch_data.mat','$(dirname $batchjob)/$(basename $batchjob .txt).mat');
M_PROG"
touch $(dirname $1)/SubmitBatchJobs_$(basename $batchjob .txt).runlimit
elif [ "${MATLABOUTOFMEMORY}" ]; then
bsub -W 8:00 -o $(dirname $batchjob)/$REPORTFILE -R 'rusage[mem=8000]' "matlab -singleCompThread -nodisplay -nojvm << M_PROG
CPCluster('$(dirname $batchjob)/Batch_data.mat','$(dirname $batchjob)/$(basename $batchjob .txt).mat');
M_PROG"
else
bsub -W 8:00 -o $(dirname $batchjob)/$REPORTFILE -R 'rusage[mem=4000]' "matlab -singleCompThread -nodisplay -nojvm << M_PROG
CPCluster('$(dirname $batchjob)/Batch_data.mat','$(dirname $batchjob)/$(basename $batchjob .txt).mat');
M_PROG"
fi
