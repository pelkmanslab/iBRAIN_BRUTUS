...
function CreateMeanMeasurmentWell_OrigRed(strBASIC)
...
for iPlate = 21:size(cellPlateNames,1)
    ...
    %zscore intensity measurment per plate
    matPlateIntensity_zscore = zscore(feature_vector);
    ...
    handles = struct();
    handles.Measurements.Well.(['Intensity',strFeatureName,strObjectName]) = [];
    handles.Measurements.Well.(['Intensity',strFeatureName,strObjectName]) = cellMeanIntensity';
    handles.Measurements.Well.(['Intensity',strFeatureName,strObjectName,'Features']) = plate_feature_vector_names;
    %strOutputFileName = fullfile(strPlatePath,['Measurements_Well_MeanVesiclesIntensity',strFeatureName,strObjectName,'.mat']);
    strOutputFileName = fullfile(strPlatePath,['Measurements_Well_Intensity',strFeatureName,'.mat']);
