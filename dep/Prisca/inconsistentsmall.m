function Y = inconsistentsmall(Z,depth)
...
Y = zeros(m,4);
   num_genes=zeros(size(Z,1)+size(Z,1)+1,1);
               num_genes(1:size(Z,1)+1)=1;
                    for(k=1:size(Z,1))
                        num_genes(size(Z,1)+1+k)=num_genes(Z(k,1))+num_genes(Z(k,2));
                    end;
                    
for k = 1:m
   
        %Only process node if it has more than 4 genes as sons
   s = zeros(4,1);
