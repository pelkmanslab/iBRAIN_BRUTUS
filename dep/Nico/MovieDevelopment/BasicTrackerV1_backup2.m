function [handles,TrackingMeasurementPrefix] = BasicTrackerV1(handles,cellAllImages,matOrderedTimePointIdx,matMetaDataInfo,strSettingBaseName)
...
handles.Measurements.(ObjectName).(strcat(TrackingMeasurementPrefix,'Features')) = {'ObjectID','ParentID','TrajectoryX','TrajectoryY','DistanceTraveled','IntegratedDistance','Linearity','Lifetime','GhostAge','FamilyIDs'};
...
for i = 1:NumberOfImageSets
   ...
   % matglobalLabelingSeq = [handles.Current.SetBeingAnalyzed , MetaData(:,SetBeingAnalyzed)', matOrderedTimePointIdx(SetBeingAnalyzed)] ;
   % else
   % matglobalLabelingSeq = [matSequenceInImagesetsForglobalLabeling ; [handles.Current.SetBeingAnalyzed, MetaData(:,SetBeingAnalyzed)', matOrderedTimePointIdx(SetBeingAnalyzed)]];
   % end
    ...
    if SetBeingAnalyzed == StartingImageSet    
       ...
       CurrentFamilyIDs = CurrentLabels;
        ...
        switch lower(TrackingMethod)
            case 'distance'
                ...
                [CurrentLabels, CurrHeaders, ParentMat, maxPreviousLabel, CurrentFamilyIDs] = EvaluateObjectOverlap(CurrentObjNhood,PreviousObjNhood,PreviousLabels,PrevHeaders,MeasuredValues,OverlapFactorC,OverlapFactorP,maxPreviousLabel,PreviousFamilyIDs);
                
            case 'overlap'  % Compute object area overlap
                MeasuredValues = ones(size(CurrentSegmentedImage));
                [CurrentLabels, CurrHeaders, ParentMat, maxPreviousLabel, CurrentFamilyIDs] = EvaluateObjectOverlap(CurrentSegmentedImage,PreviousSegmentedImage,PreviousLabels,PrevHeaders,MeasuredValues,OverlapFactorC,OverlapFactorP,maxPreviousLabel,PreviousFamilyIDs);
       ...
       %%% Determine which labels have disappeared in the current image
       [GhostLabels, gIds] = setdiff(PreviousLabels,CurrentLabels);
       ...
       for i=1:size(gIds,1)
           ...
           Ghosti = (PreviousSegmentedImageNR == gIds(i));
           if max(CurrentSegmentedImageNR(Ghosti)) == 0 && PreviousGhostAge(gIds(i)) < MaxGhostAge;
               ...
               CurrentLabels(GhostiID) = GhostLabels(i);
               ParentMat(GhostiID) = GhostLabels(i);
               CurrentFamilyIDs(GhostiID) = PreviousFamilyIDs(gIds(i));
               CurrentGhostAge(GhostiID) = PreviousGhostAge(gIds(i))+1;
       ...
       %%% In the end, the IDs of the Ghost can be allways identified by
...
%%%% SUBFUNCTIONS %%%%
%%%%%%%%%%%%%%%%%%%%%%
function [CurrentLabels, CurrentHeaders,mainParents2,maxPreviousLabel, CurrentFamilyIDs] = EvaluateObjectOverlap(CurrentLabelMatrix, PreviousLabelMatrix, PreviousLabels, PreviousHeaders, ChosenMetric,OverlapFactorC,OverlapFactorP,maxPreviousLabel,PreviousFamilyIDs)
...
if any(CurrentPreviousLabelHistogram(:)),
    ...
    % comment vito: shouldn't this be: if two parents have the same child
...
for j=1:length(idx)
    ...
    if isempty(r1);
        ...
        %VZ: create familyIDs
        CurrentFamilyIDs(idx(j)) = CurrentLabels(idx(j));
    else
        mainParents2(idx(j))=mitoParents(r1);
        %VZ: pass familyIDs
        CurrentFamilyIDs(idx(j)) = PreviousFamilyIDs(r1);
    end
end
