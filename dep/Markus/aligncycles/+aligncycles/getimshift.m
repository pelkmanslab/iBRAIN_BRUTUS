function [rowShift, columnShift] = getimshift(RefImFilename,Im2RegFilename,scaleFactor)

% GETIMSHIFT is a function that calculates the shift between two images
% in x (column) and y (row) direction. The calculation is based on 
% efficient subpixel image registration by crosscorrelation.
% For details please refer to documentation of dftregistration.m.
% INPUT:
%   RefImFilename     Filename of the reference image.
%   Im2RegFilename    Filename of the image to register.
%   scaleFactor       Upsampling factor (integer). Images will be registered 
%                     to within 1/usfac of a pixel. 
%                     For example usfac = 20 means theimages will be 
%                     registered within 1/20 of a pixel (default = 1).
% OUTPUT:
%   rowShift          Shift in pixels in row direction (y).
%   columnShift       Shift in pixels in column direction (x).


% load reference image
imRef = double(imread(RefImFilename));
%imRef = double(illcorIm((RefImFilename));
% calculate fourier transform of reference image
ftRef = fft2(imRef);

% load image to register
imReg = double(imread(Im2RegFilename));
%imReg = double(illcorIm(Im2RegFilename));
% calculate fourier transform of image to register
ftReg = fft2(imReg);

% calculate shift between reference image and image to register
if nargin >2
    matShift = dftregistration(ftRef,ftReg,scaleFactor);
else
    matShift = dftregistration(ftRef,ftReg);
end
rowShift = matShift(3);
columnShift = matShift(4);

end


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% SUBFUNCTION for efficient subpixel image registration by crosscorrelation %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
