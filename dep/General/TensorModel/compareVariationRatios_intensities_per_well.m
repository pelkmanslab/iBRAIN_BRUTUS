...
sum(Tensor.TotalCells > intMinimalBinSize)
...
%             disp('randomizing data within well')
%             matRandomizedCurrentWellTrainingData = matCurrentWellTrainingData;
%             matRandomizedCurrentWellTrainingData(:,1) = matRandomizedCurrentWellTrainingData(randperm(size(matCurrentWellTrainingData,1)),1);
...
matTrainingDataRandomized = Tensor.TrainingData;
matTrainingDataRandomized(:,1) = matTrainingDataRandomized(randperm(size(matTrainingDataRandomized,1)),1);
intMinBinSize = 50;
...
matRandomizedBinVariance = nan(1,intNumOfOkBins);
iBinCounter = 0;
...
for i = 1:Tensor.BinSizes(matBiggestVaryingDim)
    ...
    matCurrentXTrainingDataRandomized = matTrainingDataRandomized(matTrainingData(:,matBiggestVaryingDim)==i,:);
    ...
    for iBin = 1:length(matCurrentXIndices)
        ...
        matRandomizedBinVariance(iBinCounter) = nanvar(single(matCurrentXTrainingDataRandomized(matCellsInCurrentBinIndices,1)-1));%nanvar        
    end
    
    % remove processed cells from trainingdata
    matTrainingData(matTrainingData(:,matBiggestVaryingDim)==i,:) = [];
    matTrainingDataRandomized(matTrainingData(:,matBiggestVaryingDim)==i,:) = [];
end
...
nanstd(matYs) / nanmean(matVars(matCount>intMinimalBinSize))

nanvar(matYs) / nanmean(matVars(matCount>intMinimalBinSize))



figure
y=sort(matVars(matCount>50));%per well per bin variances
y2=sort(matBinVariance);%per bin variances
y3=sort(matRandomizedBinVariance);%per randomized bin variances
hold on
plot(0:(1/(length(y)-1)):1,y,'g',...
    0:(1/(length(y3)-1)):1,y3,'r','linewidth',5)
legend({'per bin','randomized'},'fontsize',20,'location','best')

% hline(nanvar(single(Tensor.TrainingData(:,1)))-1)
set(gca,'fontsize',20)
hold off
drawnow
...
figure
matEdges = 0:7:250;
y=histc(matVars(matCount>50),matEdges);%per well per bin variances
y2=histc(matBinVariance,matEdges);%per bin variances
y3=histc(matRandomizedBinVariance,matEdges);%per randomized bin variances
hold on
plot(matEdges,y / max(y),'g',...
    matEdges,y3 / max(y3),'r','linewidth',1)
legend({'per bin','randomized'},'fontsize',20,'location','best')
xlim([0,250])
% hline(nanvar(single(Tensor.TrainingData(:,1)))-1)
set(gca,'fontsize',20)
...
% nanmean(y) / nanvar(single(Tensor.TrainingData(:,1)))