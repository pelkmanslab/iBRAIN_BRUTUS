function reproduceTrainingDataCurves_probit_logtransformed(strRootPath,strFigureTitle)
...
for i = 2:iDim
    ...
    if size(matRawTotalCells{i},2) > 2
        ...
        plot(matXUnits,matCellCount(1:max(colPresent)),'Color',[.8 .8 .8],'linewidth',2)
    else
    ...
    set(gca,'XLim',matNewXLim,'FontSize',6,'YAxisLocation','right')%,'YColor',[.8 .8 .8]
%     hh = ylabel('Total number of cells','Color',[.8 .8 .8]);%,'YAxisLocation','right'
    h(1) = gca;
    

    h(2)=axes();
    boxplot(matRawLog2RIIs(:,min(colPresent):max(colPresent)),'positions',matXUnits,'colors','b')    
    ylabel('')
    xlabel('')        
...
%     end
    ...
    set(h(2),'YLim',matNewYLim,'Position',matPositionFirstAxis,'Color','none','XTick',[],'XTickLabel',[],'YAxisLocation','left','FontSize',6,'XColor','w','YColor','k','ZColor','w','YTick',[],'Box','off')    
    set(h(3),'YLim',matNewYLim,'Position',matPositionFirstAxis,'Color','none','XTick',[],'XTickLabel',[],'YAxisLocation','left','FontSize',6,'XColor','w','YColor','k','ZColor','w','Box','off')
...
%     close(hFigure(i))
    ...
    while filepresentbool
        filecounter = filecounter + 1;    
        strPrintName = fullfile(strRootPath,['ProbModel_CurveReproduction_Probit_logtransformed_',getlastdir(strRootPath),'_',num2str(filecounter)]);
        filepresentbool = fileattrib([strPrintName,'.*']);
    end
