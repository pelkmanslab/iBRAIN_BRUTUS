function reproduceTrainingDataCurves_probit(strRootPath,strFigureTitle)
...
for i = 2:iDim
    ...
    set(gca,'XLim',matNewXLim,'FontSize',6,'XTickLabel',[])
    h(1) = gca;
    ...
    h(3)=axes();
    boxplot(matModelIIs,'colors','r','labels',{})
    ylabel('')
    xlabel('')    
...
%     plot(cumsum(nanmedian(matModelExpectedInfectedCells{i})),'r')        
    ...
    set(h(3),'YLim',matNewYLim,'Position',get(h(1),'Position'),'XLim',matNewXLim,'Color','none','YAxisLocation','right','FontSize',6,'XColor','w','YColor','k','ZColor','w')
...
%     close(hFigure(i))
    ...
    while filepresentbool
        filecounter = filecounter + 1;    
        strPrintName = fullfile(strRootPath,['ProbModel_CurveReproduction_Probit_',getlastdir(strRootPath),'_',num2str(filecounter)]);
        filepresentbool = fileattrib([strPrintName,'.*']);
    end
