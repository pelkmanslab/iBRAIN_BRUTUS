...
for i = 1:intNumOfFolders
    ...
    disp(sprintf('   (TCN STEPSIZE = %.0f)',PlateTensor{i}.StepSizes(5)))       
    matStepSizesPerAssay = [matStepSizesPerAssay, PlateTensor{i}.StepSizes(5)];
    
    for iDim = 5%2:length(PlateTensor{i}.Features)
...
for i = 5
    subplot(matSubPlot(iProject,1),matSubPlot(iProject,2),matSubPlot(iProject,3))
    ...
    matRawTotalCells{i}(find(matRawTotalCells{i}<intMinimalTCN))=NaN;
...
%     %%% These two lines perform optional plate normalization
%     matRawRIIs = matRawIIs{i} ./ repmat(nanmedian(matRawIIs{i},2),1,size(matRawIIs{i},2));
%     matRawLOG2RIIs = log2(matRawRIIs);    
    


    matIndices2plot = sum(matRawIIs{i} > 0,1) > 3;

    %%% AXIS HOLDER
    h=[];
    
    hold on
    bar(nansum(matRawTotalCells{i}(:,matIndices2plot)),'FaceColor',[.8 .8 .8],'EdgeColor',[.8 .8 .8])
    title(strFigureTitle,'FontSize',12)
    set(gca,'XLim',matNewXLim,'FontSize',12)

%     matXTicks = cellfun(@str2double,cellstr(get(gca,'XTickLabel')))';
%     matNewXTicks = matXTicks * matStepSizesPerAssay(i);
%     matNewXTickLabels = arrayfun(@mat2cell,matNewXTicks);
%     matNewXTickLabels = cellfun(@num2str,matNewXTickLabels,'UniformOutput',0);
% %     cellstrXTickLabels = sprintf('%.0f ',matNewXTicks);
%     set(gca,'XTickLabel',matNewXTickLabels);
    
    h(1) = gca;

    h(2)=axes();
    boxplot(matRawIIs{i}(:,matIndices2plot),'colors','k')
    ylabel('')
    xlabel('')        
...
%     matNewYLim = [min([get(h(2),'YLim'),get(h(3),'YLim')]),max([get(h(2),'YLim'),get(h(3),'YLim')])];
    set(h(2),'YLim',matNewYLim,'Position',get(h(1),'Position'),'XLim',matNewXLim,'Color','none','XTick',[],'YAxisLocation','right','FontSize',12,'XColor','w','YColor','k','ZColor','w')    
...
%     while filepresentbool
%         filecounter = filecounter + 1;    
%         strPrintName = fullfile(strRootPath,['ProbModel_CurveReproduction_TCN_ONLY_',getlastdir(strRootPath),'_',num2str(filecounter)]);
%         filepresentbool = fileattrib([strPrintName,'.*']);
%     end
%     disp(sprintf('stored %s',strPrintName))
% 
