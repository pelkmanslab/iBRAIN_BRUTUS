...
%     for i = 2:length(cellstrNucleiFieldnames)%%% ORIGINAL
    for i = 2:5%%% SKIP THE READOUT, INFECTION, AND THE CELL TYPE CLASSIFICATIONS
...
%                     disp(sprintf('PROCESSING %s',PlateDataHandles.Measurements.Image.FileNames{k}{1,1}))
                ...
                if not(isempty(PlateDataHandles.Measurements.(strObjectName).(strCurrentFieldName){k}))
                    ...
                    if intObjectCount > 1                        
                        ...
                        matPlateData = [matPlateData;matTempData];
            ...
            matFinalData = [matFinalData, single(matPlateData)];
            size(matFinalData)
...
matCompleteData = zscore(matCompleteData);

matRndIndices = randperm(size(matCompleteData,1));
matRndIndices = matRndIndices(1:1000000);
