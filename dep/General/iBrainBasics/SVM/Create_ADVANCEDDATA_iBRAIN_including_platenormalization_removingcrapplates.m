function Create_ADVANCEDDATA_iBRAIN(path, x)	
...
%if length(x)==size(BASICDATA.(dataFields(1)), 1)

    for i=length(dataFields)
        
        BASICDATA.(dataFields(i))=BASICDATA.(dataFields(i))(x, :);
    end
%end
...
save([path,'ADVANCEDDATA2_withoutcrap'],'ADVANCEDDATA');
