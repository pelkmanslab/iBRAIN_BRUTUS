function [matImageSnake,matStitchDimensions] = get_image_snake_safia(intMaxImagePosition, strMicroscopeType)
    ...
    if nargin == 2
        if ~strcmpi(strMicroscopeType,'CW') && ...
           ...
           ~strcmpi(strMicroscopeType,'MD') && ...
            error('%s: unrecognized microscope type ''%s''. Allowed values are ''CW'', ''BD'', and ''MD''.',mfilename,strMicroscopeType)
        end
    end
